...
rem --- 2.5. Build TypeScript ---
echo   [2.5/6] Building TypeScript...
echo [%time%] [2.5/6] Building TypeScript... >> "%LOG_FILE%"
call npm run build
if errorlevel 1 (
    echo   [WARNING] TypeScript build failed!
    echo     The server may not start correctly.
    echo [%time%] [WARNING] TypeScript build failed >> "%LOG_FILE%"
) else (
    echo     TypeScript built successfully.
    echo [%time%] TypeScript build OK >> "%LOG_FILE%"
)
echo.
