...
exports.MasterAgentService = void 0;
        ...
        try {
                ...
                try {
                    ...
                    if (memSource) {
                        ...
                        for (const srcId of sourceIds.slice(0, 5)) {
                            ...
                            if (srcMem)
                                injections.push(`[来自工作流 "${srcWf.name}" 的记忆]\n${srcMem.substring(0, 5000)}`);
                        }
                    }
                    // 共享池
            ...
            try {
                ...
                for (const n of agentNodes) {
                    for (const s of SkillService.getByAgent(n.agentId)) {
                        if (!allSkills.has(s.name))
                            allSkills.set(s.name, s);
                    }
                    for (const m of McpService.getByAgent(n.agentId)) {
                        if (!allMcp.has(m.name))
                            allMcp.set(m.name, m);
                    }
                }
                if (allSkills.size > 0) {
            ...
            // Agent 类型说明
            const agentTypesInfo = '- Explore: 只读探索（搜索、分析、只读操作）\n- general-purpose: 完整权限（读写执行全权限）';
            // 检测工作流中的分叉和汇聚节点
            const forkNodes = [];
            const mergeNodes = [];
            for (const n of workflow.nodes) {
...
exports.MasterAgentService = MasterAgentService;
...
//# sourceMappingURL=MasterAgentService.js.map