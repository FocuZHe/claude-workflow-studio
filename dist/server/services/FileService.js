...
class FileService {
    ...
    static runtimeWorkspaceRoot = null;
    /** 按 runId 追踪的活跃执行 */
     ...
     */
    static get isWorkflowExecution() {
     ...
     */
    static setWorkspaceRoot(newRoot) {
        const resolved = path_1.default.resolve(newRoot);
        ...
        if (loadedState?.workflows?.length > 0 && loadedState?.manifest?.workspaceId) {
            ...
            let migrated = false;
            loadedState.workflows = loadedState.workflows.map((w) => {
            ...
            if (migrated) {
                try {
                    const wfPath = path_1.default.join(resolved, 'WORKFLOWS', 'workflows.json');
        ...
        if (loadedState) {
            try {
                ...
                if (loadedState.chatSessions && loadedState.chatSessions.length > 0) {
                    const globalChats = ChatSessionModel.findAll({ page: 1, limit: 99999 }).items.filter((c) => !c.workspaceId);
                    ChatSessionModel.clear();
                    ChatSessionModel.reload(loadedState.chatSessions);
                    if (globalChats.length > 0)
                        ChatSessionModel.reload(globalChats);
                }
        ...
        const services = [
            { name: 'ArtifactIndexService', init: (r) => { const s = require('./ArtifactIndexService'); if (s.stopWatching)
                    s.stopWatching(); s.init(r); } },
            { name: 'CheckpointService', init: (r) => require('./CheckpointService').init(r) },
            { name: 'ReportService', init: (r) => require('./ReportService').init(r) },
            { name: 'MemoryService', init: (r) => require('./MemoryService').init(r) },
            { name: 'SnapshotService', init: (r) => require('./SnapshotService').init(r) },
            { name: 'TagService', init: (r) => require('./TagService').init(r) },
            { name: 'KnowledgeService', init: (r) => require('./KnowledgeService').init(r) },
        ];
     ...
     */
    static setExecutionMode(runId, excludedPaths = []) {
     ...
     * @param runId - 执行的唯一标识
     */
    static clearExecutionMode(runId) {
    ...
    static resolvePath(inputPath) {
        ...
        const resolved = path_1.default.resolve(workspaceRoot, inputPath);
        // Check execution mode isolation — block excluded paths (e.g. WORKFLOWS data dir)
        if (FileService.isWorkflowExecution) {
            for (const excluded of FileService.excludedPaths) {
                const normalizedExcluded = path_1.default.resolve(excluded).replace(/\\/g, '/');
     ...
     */
    static resolveBrowsingPath(inputPath) {
        const root = FileService.getWorkspaceRoot();
        if (!inputPath) {
            return root ? path_1.default.resolve(root) : null;
        }
    ...
    static getRelativePath(absolutePath) {
        ...
        return path_1.default.relative(normalizedRoot, absolutePath).replace(/\\/g, '/');
    }
    /**
     * List directory contents
     */
    static listDirectory(relativePath) {
        const workspaceRoot = FileService.getWorkspaceRoot();
        if (!workspaceRoot) {
     ...
     * Write/create file
     */
    static writeFile(relativePath, content) {
        const fullPath = FileService.resolvePath(relativePath);
        if (!fullPath) {
     ...
     */
    static deletePath(relativePath) {
        const fullPath = FileService.resolvePath(relativePath);
        if (!fullPath || !fs_1.default.existsSync(fullPath)) {
    ...
    static renamePath(oldPath, newPath) {
        ...
        if (!fullOldPath || !fs_1.default.existsSync(fullOldPath)) {
            throw new AppError('NOT_FOUND', `Path '${oldPath}' not found`, 404);
        }
        if (fullNewPath && fs_1.default.existsSync(fullNewPath)) {
        ...
        const dir = path_1.default.dirname(fullNewPath);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        fs_1.default.renameSync(fullOldPath, fullNewPath);
     ...
     * Create workspace
     */
    static createWorkspace(name, template, parentPath) {
        ...
        if (parentPath && (path_1.default.isAbsolute(normalizedParent) || /^[A-Za-z]:/.test(normalizedParent))) {
            const resolved = path_1.default.resolve(parentPath);
            if (!fs_1.default.existsSync(resolved)) {
        ...
        const workspacePath = path_1.default.join(parentDir, sanitizedName);
        ...
        if (template === 'basic') {
            fs_1.default.mkdirSync(path_1.default.join(workspacePath, 'src'), { recursive: true });
            fs_1.default.writeFileSync(path_1.default.join(workspacePath, 'README.md'), `# ${name}\n\nCreated by Multi-Agent Platform\n`);
        }
        // For workspaces outside the workspace root, return absolute path
        const root = FileService.getWorkspaceRoot();
        const normalizedRoot = root ? path_1.default.resolve(root) : null;
     ...
     */
    static browseDirectories(inputPath) {
        ...
        const isAbsolute = normalizedInput && (path_1.default.isAbsolute(normalizedInput) || /^[A-Za-z]:/.test(normalizedInput));
        ...
        if (!inputPath) {
            ...
            const fullPath = path_1.default.resolve(workspaceRoot);
            const normalizedRoot = path_1.default.resolve(workspaceRoot);
            const entries = fs_1.default.readdirSync(fullPath, { withFileTypes: true });
            const directories = entries
                .filter(e => e.isDirectory())
                .map(e => ({
                name: e.name,
                path: path_1.default.relative(normalizedRoot, path_1.default.join(fullPath, e.name)).replace(/\\/g, '/')
            }))
        ...
        let fullPath;
        if (isAbsolute) {
            fullPath = path_1.default.resolve(inputPath);
        }
        else {
            if (!workspaceRoot) {
                throw new AppError('VALIDATION_ERROR', '没有活跃工作区，无法使用相对路径', 400);
            }
            fullPath = path_1.default.resolve(workspaceRoot, inputPath);
        }
        if (!fs_1.default.existsSync(fullPath)) {
            throw new AppError('NOT_FOUND', `Directory '${inputPath}' not found`, 404);
        }
        ...
        const normalizedRoot = workspaceRoot ? path_1.default.resolve(workspaceRoot) : null;
        ...
        const directories = entries
            ...
            return {
                name: entry.name,
                path: useRelative ? path_1.default.relative(normalizedRoot, entryPath).replace(/\\/g, '/') : entryPath
            };
        })
            .sort((a, b) => a.name.localeCompare(b.name));
        ...
        if (useRelative) {
            ...
            currentPath = fullPath === normalizedRoot
                ? ''
                : path_1.default.relative(normalizedRoot, fullPath).replace(/\\/g, '/');
            const parentAbs = path_1.default.dirname(fullPath);
            parentPath = parentAbs === normalizedRoot
                ? ''
                : path_1.default.relative(normalizedRoot, parentAbs).replace(/\\/g, '/');
        }
        else {
            // Absolute paths
            currentPath = fullPath;
            parentPath = path_1.default.dirname(fullPath) !== fullPath ? path_1.default.dirname(fullPath) : null;
        }
        return {
            currentPath,
            parentPath,
    ...
    static _browseRoot() {
            ...
            for (let i = 65; i <= 90; i++) {
                ...
                try {
                    if (fs_1.default.existsSync(drivePath)) {
                        fs_1.default.accessSync(drivePath, fs_1.default.constants.R_OK);
        ...
        const entries = fs_1.default.readdirSync('/', { withFileTypes: true });
        const directories = entries
            .filter(e => e.isDirectory())
     ...
     */
    static isWithinWorkspace(targetPath) {
        const root = FileService.getWorkspaceRoot();
        if (!root)
            return false;
        const workspaceRoot = path_1.default.resolve(root);
