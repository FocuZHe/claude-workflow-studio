...
class TaskService {
    ...
    static create(data) {
        ...
        if (data.workspaceId && !data.workspaceName) {
            try {
                ...
                if (ws)
                    data.workspaceName = ws.name || ws.path?.split(/[\\/]/).pop() || '';
            }
            catch (e) { /* ignore */ }
        }
    ...
    static async execute(id) {
        ...
        if (!TaskModel.isValidTransition(task.status, 'running')) {
            throw new AppError('VALIDATION_ERROR', `Cannot execute task in '${task.status}' status`, 400);
        }
    ...
    static async _executeWhenReady(taskId) {
        ...
        while (Date.now() - start < maxWait) {
            ...
            if (!wf || wf.executionStatus !== 'running')
                break;
    ...
    static async _executeViaAgent(task) {
        ...
        for (let attempt = 0; attempt <= totalAttempts; attempt++) {
            try {
                ...
                const output = await claudeService.execute(taskRunId, task.assignedAgentId || null, modifiedInput, config);
                return output;
            }
            catch (err) {
                ...
                if (SelfRepair.isNonRetryable(err))
                    throw err;
            ...
            const checkInterval = setInterval(() => {
                try {
                    ...
                    if (workflow.executionStatus === 'completed') {
                        ...
                        const output = endResult && endResult.length > 0
    ...
    static cancel(id) {
        ...
        if (!TaskModel.isValidTransition(task.status, 'cancelled')) {
            throw new AppError('VALIDATION_ERROR', `Cannot cancel task in '${task.status}' status`, 400);
        }
    ...
    static pause(id) {
        ...
        if (task.workflowId) {
            try {
                ...
                logger.warn(`Failed to pause workflow: ${e.message}`);
            }
        }
    ...
    static resume(id) {
        ...
        if (task.status !== 'paused')
            throw new AppError('VALIDATION_ERROR', '只能恢复已暂停的任务', 400);
        ...
        if (task.workflowId) {
            try {
                ...
                logger.warn(`Failed to resume workflow: ${e.message}`);
            }
        }
