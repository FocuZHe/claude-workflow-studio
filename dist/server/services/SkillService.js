...
class SkillService {
    static skills = [...BUILTIN_SKILLS];
    static installedSkills = [];
    /**
     * 确保 skills 目录存在
     */
    static ensureSkillsDir() {
     ...
     * 创建 SKILL.md 文件
     */
    static createSkillFile(skillId, name, description) {
...
`;
     ...
     * 删除 SKILL.md 文件
     */
    static removeSkillFile(skillId) {
        const skillDir = path.join(SKILLS_DIR, skillId);
        if (fs.existsSync(skillDir)) {
     ...
     * 获取Agent已安装的技能ID列表
     */
    static getSkillIdsByAgent(agentId) {
     ...
     */
    static install(skillId, agentId) {
        if (!agentId) {
