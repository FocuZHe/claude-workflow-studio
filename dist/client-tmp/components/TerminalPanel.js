...
window.TerminalPanel = (() => {
    ...
    async function closeSession(id) {
        ...
        if (activeSessionId) {
            ...
            const container = document.getElementById('terminal-xterm-container');
            if (container)
                container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);">无活动会话</div>';
        }
        saveState();
    }
    ...
    function stopResize() {
        ...
        localStorage.setItem('terminal-panel-height', String(panel.offsetHeight));
    }
