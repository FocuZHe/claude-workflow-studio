...
window.WorkflowReadOnlyView = (() => {
    ...
    function init(workflow, options = {}) {
        ...
        document.querySelector('.ro-goto-edit')?.addEventListener('click', () => {
            if (options.onGotoEdit)
                options.onGotoEdit(workflow?.id);
        });
    }
    // ── Render Nodes ──
    function renderNodes() {
      ...
      `;
        ...
        layer.querySelectorAll('.ro-node').forEach((el) => {
            el.addEventListener('click', () => showNodeDetail(el.dataset.id || null));
        });
    }
    // ── Render Edges ──
    ...
    function updateTransform() {
        ...
        if (g)
            g.setAttribute('transform', `translate(${_pan.x},${_pan.y}) scale(${_zoom})`);
    }
    ...
    function updateExecutionData(executionData) {
        ...
        if (textEl)
            textEl.textContent = `${completedCount}/${_nodes.length} (${progressPercent}%)`;
        if (_selectedNodeId)
