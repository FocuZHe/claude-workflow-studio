...
window.WorkflowToolbar = (() => {
    ...
    function render(workflow, executionData) {
        if (!workflow)
            return '<div class="toolbar" style="opacity:0.5;"><span style="font-size:12px;color:var(--text-muted);">请选择一个工作流开始编辑</span></div>';
        ...
        return `
      <div class="toolbar" style="flex-wrap:wrap;gap:6px 0;">
        ...
        <div class="toolbar-group">
          ...
          ` : `
            <button class="btn btn-sm btn-success" id="wf-create-task" title="创建任务执行此工作流">创建任务</button>
          `}
        </div>
        <div class="toolbar-separator"></div>
        <div class="toolbar-group">
        ...
        <span style="font-size:11px;color:var(--text-muted);font-family:var(--font-mono);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px;margin-left:auto;">
        ...
        <div class="toolbar-separator"></div>
        <div class="toolbar-group">
          <button class="btn btn-sm ${workflow.memoryEnabled ? 'btn-primary' : 'btn-secondary'}" id="wf-memory-settings" title="${workflow.memoryEnabled ? '记忆注入已开启 - 点击配置' : '记忆注入已关闭 - 点击开启'}">
            ${workflow.memoryEnabled ? '🧠 记忆 ON' : '🧠 记忆'}
          </button>
    ...
    function init(workflow, executionData, cbs) {
        ...
        document.getElementById('wf-create-task')?.addEventListener('click', () => callbacks.onCreateTask?.());
        ...
        document.getElementById('wf-import-md-btn')?.addEventListener('click', () => {
            ...
            input.onchange = async (e) => {
                ...
                try {
                    ...
                    const res = await window.API.importWorkflowMd(content, null, store.get?.('currentWorkspaceId'));
                    if (res.success) {
                    ...
                    window.Toast.error('导入失败: ' + (err.message || '未知错误'));
                }
            };
            input.click();
        });
        ...
        document.getElementById('wf-export-md-btn')?.addEventListener('click', async () => {
            if (!callbacks.onGetCurrentWorkflowId)
                return;
            const wfId = callbacks.onGetCurrentWorkflowId();
            if (!wfId) {
                window.Toast.warning('请先保存工作流');
                return;
            }
            try {
                const url = window.API.getWorkflowExportMdUrl(wfId);
                ...
                window.Toast.success('工作流已导出');
            }
            catch (err) {
                window.Toast.error('导出失败: ' + (err.message || '未知错误'));
            }
        });
    }
    function autoLayout() {
        const nodes = window.WorkflowCanvas.getNodes();
        const edges = window.WorkflowCanvas.getEdges();
        if (nodes.length === 0)
            return;
        ...
        const startX = 60;
        const startY = 60;
        Object.keys(layerGroups).sort((a, b) => Number(a) - Number(b)).forEach((layerIdx) => {
        ...
        window.WorkflowCanvas.loadWorkflow({ nodes, edges });
    }
