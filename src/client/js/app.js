...
(function () {
  ...
  // Cleanup on route change — only clean up the previous page
  ...
  // ── Font scaling on resize (only affects font-size, no layout calc) ──
  function updateFontScale() {
    // Use screen diagonal (in CSS pixels) to detect physically larger monitors
    ...
    const baseFontRef = isLargeNative
      ...
      : 15;
    // Use viewport diagonal — scales based on both width and height
    const vpDiag = Math.sqrt(window.innerWidth ** 2 + window.innerHeight ** 2);
    const refDiag = 2203; // diagonal of 1920×1080 reference
