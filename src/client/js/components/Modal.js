...
window.Modal = (() => {
  ...
  function setContent({ title, body, footer }) {
    if (!overlay) return;
    if (title) overlay.querySelector('.modal-title').innerHTML = title;
    if (body) overlay.querySelector('.modal-body').innerHTML = body;
    if (footer !== undefined) {
   ...
   * @param {boolean} [danger=false] - If true, message text is shown in red
