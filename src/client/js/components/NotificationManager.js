...
window.NotificationManager = (() => {
  ...
  function listen() {
    ...
    WS.on('files.misplaced', (payload) => {
      if (!payload.misplacedFiles || payload.misplacedFiles.length === 0) return;
  ...
  function showApprovalModal(payload) {
    const { workflowId, approvalRequestId, title, description, context, timeout } = payload;
    ...
    setTimeout(() => {
      document.querySelector('.approval-approve-btn')?.addEventListener('click', async () => {
        ...
        try { await API.respondApproval(approvalRequestId, 'approve', comment); Toast.success('已通过'); Modal.close(); } catch (e) { Toast.error(e.message); }
      });
      document.querySelector('.approval-reject-btn')?.addEventListener('click', async () => {
        const comment = prompt('拒绝原因:') || '';
        if (!comment) return;
        try { await API.respondApproval(approvalRequestId, 'reject', comment); Toast.info('已拒绝'); Modal.close(); } catch (e) { Toast.error(e.message); }
      });
    }, 100);
