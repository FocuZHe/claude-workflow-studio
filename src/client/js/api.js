...
window.API = (() => {
  const BASE = '/api';
  // Sentinel promise that never settles — silences stale requests completely
  const NEVER = new Promise(() => {});

  // Auto-load API key from localStorage or fetch from server
  ...
  async function request(method, path, body = null, params = null) {
    ...
    // Wait for API key if still fetching
    ...
    // Capture navigation sequence — if navigation changes, hang the promise.
    // navSeq=0 means no navigation has happened yet (app init), never treat as stale.
    ...
    if (!res.ok || data.success === false) {
      ...
      // If 401/403 and we had a key, clear it and retry once with fresh key
  ...
  return {
    ...
    // API Keys (multi-config)
    getApiConfigs: () => get('/keys'),
    createApiConfig: (data) => post('/keys', data),
    updateApiConfig: (id, data) => put(`/keys/${id}`, data),
    deleteApiConfig: (id) => del(`/keys/${id}`),
    setDefaultApiConfig: (id) => put(`/keys/${id}/default`),
    testApiConfig: (id) => get(`/keys/${id}/test`),
    getApiKey: (id) => get(`/keys/${id}/key`),
    ...
    // Skills (global pool, no per-agent install)
    ...
    // MCP Tools (removed — managed via Claude Code CLI global config)
    ...
    // Chain Invocation (Subworkflow selection)
