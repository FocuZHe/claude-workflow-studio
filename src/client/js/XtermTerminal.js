...
window.XtermTerminal = (() => {
  /** @type {Map<string, {terminal: object, fitAddon: object, disposables: Function[]}>} */
  const instances = new Map();

  /** @type {Function|null} Pending create calls waiting for xterm to load */
  let _pendingCreates = [];
   ...
   * @returns {{ terminal, fitAddon }|null}
   */
  function create(sessionId, container) {
    ...
    const entry = { terminal, fitAddon, disposables: [() => resizeObserver.disconnect()] };
   ...
   */
  function onInput(sessionId, callback) {
    const entry = instances.get(sessionId);
    if (entry) {
