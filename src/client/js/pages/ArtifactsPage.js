...
window.ArtifactsPage = (() => {
    ...
    async function batchDelete() {
        ...
        if (!confirm(`确定删除选中的 ${_selected.size} 个文件？此操作将同时删除实际文件。`))
            return;
        let ok = 0, fail = 0;
    ...
    function bindCardListeners(listEl) {
        ...
        listEl.querySelectorAll('.artifact-delete-btn').forEach((btn) => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!confirm(`确定删除文件 "${name}"？此操作将同时删除实际文件。`))
                    return;
                try {
                    await API.deleteArtifact(id);
        ...
        listEl.querySelectorAll('.artifact-select-cb').forEach((cb) => {
            ...
            cb.addEventListener('change', () => {
                const checkbox = cb;
                if (checkbox.checked)
                    _selected.add(checkbox.dataset.id);
                else
                    _selected.delete(checkbox.dataset.id);
    ...
    function updateSelectUI() {
        ...
        if (selectBtn)
            selectBtn.style.display = _selectionMode ? 'none' : '';
        if (deleteBtn)
            deleteBtn.style.display = _selectionMode ? '' : 'none';
        if (cancelBtn)
            cancelBtn.style.display = _selectionMode ? '' : 'none';
    }
    function cleanup() {
        _artifacts = [];
        _query = '';
