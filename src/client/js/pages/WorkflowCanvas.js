...
window.WorkflowCanvas = (() => {
    ...
    function onClaudeStream(payload) {
        ...
        if (!_currentWorkflowId || payload.workflowId !== _currentWorkflowId)
            return;
    ...
    function updateTransform() {
        ...
        if (g)
            g.setAttribute('transform', `translate(${panOffset.x},${panOffset.y}) scale(${zoom})`);
    }
    // ── Render All ──
    function renderAll() {
        renderEdges();
    ...
    function bindNodeEvents() {
        ...
        layer.querySelectorAll('.wf-node').forEach(el => {
            ...
            nodeEl.addEventListener('mousedown', (e) => {
                if (e.target.classList.contains('port-in') || e.target.classList.contains('port-out'))
                    return;
                if (connecting) {
                    cancelConnecting();
                    return;
            ...
            nodeEl.addEventListener('click', (e) => {
                ...
                const nodeData = nodes.find(n => n.id === nodeEl.dataset.id);
                if (nodeData) {
                    const nx = (nodeData.position?.x || 0) * zoom + panOffset.x + NODE_W * zoom / 2;
                    const ny = (nodeData.position?.y || 0) * zoom + panOffset.y;
                    showDeleteToolbar('node', nodeEl.dataset.id, nx, ny);
                }
                showNodeDetail(nodeEl.dataset.id);
                // Show error detail panel for failed nodes
                if (nodeData && nodeData.status === 'failed') {
                    showErrorDetailPanel(nodeEl.dataset.id);
                }
            });
            // Right-click context menu
            nodeEl.addEventListener('contextmenu', (e) => {
                e.preventDefault();
                showNodeContextMenu(nodeEl.dataset.id, e.clientX, e.clientY);
            });
        });
    }
    ...
    function drawTempConnection(e) {
        ...
        // 使用 SVG CTM 矩阵转换鼠标坐标，兼容浏览器缩放
        const pt = svgEl.createSVGPoint();
        pt.x = e.clientX;
        pt.y = e.clientY;
        const canvasGroup = document.getElementById('canvas-group');
        const ctm = canvasGroup?.getScreenCTM()?.inverse();
        let tx, ty;
        if (ctm) {
            ...
            // 降级方案
            const rect = svgEl.getBoundingClientRect();
            tx = (e.clientX - rect.left - panOffset.x) / zoom;
            ty = (e.clientY - rect.top - panOffset.y) / zoom;
        }
        const dx = tx - sx;
    ...
    function computeEdgePath(src, tgt, edge) {
        ...
        if (!edge?.bendOffset) {
            ...
            for (const n of nodes) {
                ...
                if (nx + NODE_W <= corridorLeft || nx >= corridorRight)
                    continue;
                const nodeTop = ny - 4;
                ...
                if (edgeYAtNode > nodeTop && edgeYAtNode < nodeBottom) {
                    ...
                    if (Math.abs(needed) > Math.abs(autoNudge))
                        autoNudge = needed;
                }
            }
        }
      ...
      `;
        ...
        layer.querySelectorAll('.edge-hit').forEach(el => {
            ...
            hitEl.addEventListener('mousedown', (e) => {
                e.stopPropagation();
                const edge = edges.find(ed => ed.id === hitEl.dataset.id);
                if (!edge)
                    return;
                const rect = svgEl.getBoundingClientRect();
                draggingEdge = {
            ...
            hitEl.addEventListener('contextmenu', (e) => {
                ...
                if (edge) {
                    ...
                    showDeleteToolbar('edge', hitEl.dataset.id, x, y);
                }
            });
      ...
      <div style="padding:16px;">
        ...
        ${(node.type === 'agent' || !node.type || node.type === 'developer' || node.type === 'reviewer' || node.type === 'tester' || node.type === 'planner' || node.type === 'debugger' || node.type === 'documenter') ? `
          ...
          <div class="form-group" style="margin-top:8px;">
            <div class="form-label">Agent 类型</div>
            <select id="detail-agent-type" style="width:100%;padding:6px 10px;border:1px solid var(--border-subtle);border-radius:4px;background:var(--bg-deep);color:var(--text-primary);font-size:12px;">
              <option value="general-purpose" ${node.agentType === 'general-purpose' || node.agentType === 'general' || node.agentType === 'build' || node.agentType === 'test' || node.agentType === 'doc' ? 'selected' : ''}>⚙️ 通用 Agent (完整权限)</option>
              <option value="Explore" ${node.agentType === 'Explore' || node.agentType === 'explore' ? 'selected' : ''}>🔍 探索 Agent (只读权限)</option>
            </select>
            <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">
              选择 Agent 类型将自动配置权限和系统提示
            </div>
          </div>
          <div class="form-group">
        ...
        ${node.type === 'condition' ? `
          <div class="form-group">
            <div class="form-label">判断依据</div>
            <textarea id="detail-cfg-condition-pattern" rows="3" placeholder="描述判断条件，AI将根据上游输出和此描述进行判断"
                  style="width:100%;padding:6px 10px;border:1px solid var(--border-subtle);border-radius:4px;background:var(--bg-deep);color:var(--text-primary);font-size:12px;font-family:var(--font-mono);resize:vertical;">${escapeXml(node.config?.pattern || '')}</textarea>
            <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">
              AI将根据上游节点输出和此判断依据，自动决定走哪个分支
            </div>
          </div>
          <div class="form-group" style="margin-top:8px;">
    ...
    function bindDetailConfigSave(node) {
        ...
        document.getElementById('detail-save-agent-prompt')?.addEventListener('click', () => {
            ...
            node.agentType = document.getElementById('detail-agent-type')?.value || 'general';
            // 更新节点标签为所选智能体名称
    ...
    `;
        switch (type) {
          ...
          <div class="form-group" style="margin-top:8px;">
            <div class="form-label">判断依据</div>
            <textarea id="node-cfg-condition-pattern" class="form-input" rows="3" placeholder="描述判断条件，AI将根据上游输出和此描述进行判断" style="width:100%;padding:6px 10px;border:1px solid var(--border-subtle);border-radius:4px;background:var(--bg-deep);color:var(--text-primary);font-size:12px;font-family:var(--font-mono);resize:vertical;"></textarea>
            <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">
              AI将根据上游节点输出和此判断依据，自动决定走哪个分支
            </div>
          </div>
        ...
        `;
                break;
            default:
                // agent type: agent selector + agentType + defaultPrompt + requiresInput
                fieldsHtml += `
          <div class="form-group" style="margin-top:8px;">
          ...
          <div class="form-group" style="margin-top:8px;">
            <div class="form-label">Agent 类型</div>
            <select id="node-cfg-agent-type" style="width:100%;padding:6px 10px;border:1px solid var(--border-subtle);border-radius:4px;background:var(--bg-deep);color:var(--text-primary);font-size:12px;">
              <option value="general-purpose">⚙️ 通用 Agent (完整权限)</option>
              <option value="Explore">🔍 探索 Agent (只读权限)</option>
            </select>
            <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">
              选择 Agent 类型将自动配置权限和系统提示
            </div>
    ...
    function createNodeWithType(type) {
        ...
        if (type === 'agent' || type === 'developer' || type === 'reviewer' || type === 'tester' ||
            ...
            nodeData.agentType = document.getElementById('node-cfg-agent-type')?.value || 'general';
        }
        nodes.push(nodeData);
        _onEdit?.();
        renderAll();
    ...
    function showTestNodeDialog(nodeId) {
      ...
      `,
        ...
        document.getElementById('test-node-confirm')?.addEventListener('click', async () => {
            ...
            if (resultEl)
                resultEl.innerHTML = '<div style="color:var(--text-muted);font-size:12px;">正在执行测试...</div>';
            try {
                // Get workflow id from parent page
                const wfId = window.WorkflowsPage?.getCurrentWorkflowId?.();
                if (!wfId) {
                    if (resultEl)
                        resultEl.innerHTML = '<div style="color:var(--accent-red);font-size:12px;">无法获取工作流 ID</div>';
                    return;
                }
          ...
          `;
                ...
                if (resultEl)
                    resultEl.innerHTML = `<div style="color:var(--accent-red);font-size:12px;">测试失败: ${escapeXml(e.message)}</div>`;
            }
        });
    }
    ...
    async function inlineWorkflow(wfId) {
        try {
            ...
            for (const edge of (wf.edges || [])) {
                ...
                if (!srcNode || srcNode.type === 'start' || !tgtNode || tgtNode.type === 'end')
                    continue;
                newEdges.push({
