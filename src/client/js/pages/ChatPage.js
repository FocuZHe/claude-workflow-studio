...
window.ChatPage = (() => {
  ...
  function handleSlashDropdown() {
    ...
    const matches = SLASH_COMMANDS.filter(c =>
      c.cmd.startsWith(query) || c.desc.toLowerCase().includes(query.slice(1))
    );
    ...
    dropdown.querySelectorAll('.slash-item').forEach(el => {
      el.addEventListener('click', () => selectSlashCommand(el.dataset.cmd));
      el.addEventListener('mouseenter', () => {
        dropdown.querySelectorAll('.slash-item').forEach(s => {
          s.classList.remove('selected');
        });
        el.classList.add('selected');
        slashIndex = Array.from(dropdown.children).indexOf(el);
      });
    });
  ...
  function renderSessionList(list) {
    ...
    container.querySelectorAll('.chat-session-item').forEach(el => {
      el.addEventListener('click', () => {
        ...
        _clickTimer = setTimeout(() => switchSession(el.dataset.id), 250);
      });
      el.addEventListener('mouseenter', () => {
        const delBtn = el.querySelector('.session-delete-btn');
    ...
    container.querySelectorAll('.session-title').forEach(titleEl => {
      titleEl.addEventListener('dblclick', (e) => {
        ...
        input.addEventListener('keydown', (ev) => {
          ...
          if (ev.key === 'Escape') { _editingTitle = false; input.value = currentTitle; input.blur(); }
        });
      });
    });
  ...
  async function createSession() {
    try {
      ...
      if (globalModel) payload.model = globalModel;
  ...
  async function switchSession(id) {
    ...
    const oldStreamingEl = container?.querySelector('.chat-message.streaming');
    if (oldStreamingEl) oldStreamingEl.remove();
  ...
  async function sendMessage() {
    if (activeSessionId && getStreamingState(activeSessionId).isStreaming) return;
    ...
    try {
      ...
      const streamingEl = container?.querySelector('.chat-message.streaming');
  ...
  async function handleSlashCommand(content) {
    ...
    if (cmd === '/model') {
      ...
      try {
        ...
        if (activeSessionMeta) activeSessionMeta.model = args.trim();
    ...
    if (cmd === '/system') {
      ...
      try {
        ...
        if (activeSessionMeta) activeSessionMeta.systemPrompt = args.trim();
    ...
    if (cmd === '/config') {
      ...
      try {
        ...
        if (activeSessionMeta) activeSessionMeta[key] = value;
