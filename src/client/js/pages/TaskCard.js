...
window.TaskCard = (() => {
  ...
  function updateCardStatus(taskId, newStatus) {
    ...
    if (newStatus === 'running' && !indicatorEl) {
      ...
      if (badgeParent) badgeParent.insertBefore(span, badgeParent.children[1]);
    ...
    let btnsHtml = '';
    if (newStatus === 'pending') btnsHtml += `<button class="btn btn-sm btn-success btn-execute" title="执行">${Icon.svg('play', 14)}</button>`;
    if (newStatus === 'running') btnsHtml += `<button class="btn btn-sm btn-secondary btn-pause" title="暂停">${Icon.svg('pause', 14)}</button>`;
    if (newStatus === 'paused') btnsHtml += `<button class="btn btn-sm btn-secondary btn-resume" title="继续">${Icon.svg('play', 14)}</button>`;
    if (newStatus === 'running' || newStatus === 'paused') btnsHtml += `<button class="btn btn-sm btn-danger btn-cancel" title="取消">${Icon.svg('stop', 14)}</button>`;
