// ═══════════════════════════════════════════════
// TaskCreate — Creation Form Modal (with batch mode)
// ═══════════════════════════════════════════════
...
window.TaskCreate = (() => {
  ...
  function bindEvents(agents, onSubmit) {
    ...
    document.getElementById('batch-mode-toggle')?.addEventListener('change', (e) => {
      ...
      if (singleForm) singleForm.style.display = _isBatchMode ? 'none' : 'block';
      if (batchForm) batchForm.style.display = _isBatchMode ? 'block' : 'none';
      if (submitBtn) submitBtn.textContent = _isBatchMode ? '创建队列' : '创建任务';
    });
  ...
  async function loadWorkspaceOptions(selectId) {
    ...
    try {
      ...
      wsSelect.innerHTML = activeList.map(ws =>
        `<option value="${ws.id}" data-path="${escapeHtml(ws.path)}" ${ws.path === currentPath ? 'selected' : ''}>${escapeHtml(ws.name || ws.path.split(/[/\\]/).pop())}</option>`
      ).join('');
  ...
  function bindBatchItemEvents() {
    document.querySelectorAll('.batch-item-text').forEach(el => {
      el.addEventListener('input', (e) => {
        ...
        if (_batchItems[idx]) _batchItems[idx].input = e.target.value;
      });
    });
  ...
  function syncBatchItemValues() {
    document.querySelectorAll('.batch-item-text').forEach(el => {
      ...
      if (_batchItems[idx]) _batchItems[idx].input = el.value;
    });
  }
  ...
  function handleSubmitBatch(onSubmit) {
    ...
    if (!wsId) { Toast.warning('请选择工作区'); return; }
