...
window.WorkflowsPage = (() => {
    ...
    async function render() {
                ...
                if (footer) {
                    footer.querySelectorAll('[data-leave-choice]').forEach((btn) => {
                        btn.addEventListener('click', () => {
                            resolve(btn.dataset.leaveChoice || 'cancel');
                            Modal.close();
                        });
                    });
                }
      ...
      `;
        ...
        _wsUnsubs.push(WS.on('workflow.created', () => loadWorkflows()));
        _wsUnsubs.push(WS.on('workflow.updated', () => loadWorkflows()));
        _wsUnsubs.push(WS.on('workflow.deleted', () => loadWorkflows()));
    ...
    async function loadWorkflows() {
        try {
            ...
            if (typeof Store !== 'undefined')
                Store.set('workflowsDirty', Date.now());
        }
        catch (e) {
            Toast.error('加载工作流失败');
        }
    ...
    function getWorkspaceName(workspaceId) {
        ...
        return ws ? (ws.name || ws.path.split(/[/\\]/).pop() || null) : null;
    }
    ...
    function updateViewModeButtons() {
        ...
        if (builderBtn)
            builderBtn.classList.toggle('active', viewMode === 'builder' || viewMode === 'readonly');
    }
          ...
          <div class="card card-workflow hover-lift card-enter" data-id="${wf.id}" style="position:relative;${_selectionMode ? 'padding-left:40px;' : ''}">
            ...
            <div class="card-body">
              ...
              <div style="display:flex;gap:12px;align-items:center;font-size:11px;color:var(--text-muted);font-family:var(--font-mono);">
                ...
                ${wf.memoryEnabled ? '<span style="color:var(--accent-green);">🧠 记忆</span>' : ''}
              </div>
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:12px;">
              <button class="btn btn-sm btn-secondary wf-edit" data-id="${wf.id}">${Icon.svg('edit', 14)} 编辑</button>
              <button class="btn btn-sm ${wf.memoryEnabled ? 'btn-primary' : 'btn-ghost'} wf-toggle-memory" data-id="${wf.id}" data-enabled="${wf.memoryEnabled ? '1' : '0'}" title="${wf.memoryEnabled ? '点击关闭记忆注入' : '点击开启记忆注入'}">${Icon.svg('memory', 16)}</button>
    ...
    `;
        ...
        container.querySelectorAll('.wf-toggle-memory').forEach((b) => b.addEventListener('click', async (e) => {
            e.stopPropagation();
            const id = b.dataset.id;
            const currentlyEnabled = b.dataset.enabled === '1';
            try {
                await API.updateWorkflow(id, { memoryEnabled: !currentlyEnabled });
                Toast.success(currentlyEnabled ? '记忆注入已关闭' : '记忆注入已开启');
                await loadWorkflows();
                renderContent();
            }
            catch (err) {
    ...
    function initToolbar() {
        ...
        WorkflowToolbar.init(currentWorkflow, executionData, {
            onSave: saveWorkflow,
            onCreateTask: () => createTaskForWorkflow(currentWorkflow),
    ...
    async function createWorkflow() {
        ...
        try {
            ...
            currentWs = wsRes.data?.state?.workspacePath;
        }
        catch (e) { /* ignore */ }
        if (!currentWs) {
            Toast.warning('请先激活一个工作区，再创建工作流');
    ...
    async function renameWorkflow(id) {
        ...
        if (!newName || !newName.trim() || newName.trim() === wf.name)
            return;
        try {
        ...
        try {
            ...
            if (_isDraft) {
                ...
                if (currentWorkflow._createAll) {
                    ...
                    const totalWs = res.meta?.totalWorkspaces || 1;
                    Toast.success(`工作流已在 ${totalWs} 个工作区创建`);
    ...
    async function openMemorySettings(workflow) {
        ...
        <div style="font-size:13px;color:var(--text-secondary);margin-bottom:12px;">
          控制工作流执行时是否注入历史记忆。默认关闭以节省Token。
        </div>
        <div style="display:flex;flex-direction:column;gap:8px;">
          <label style="display:flex;align-items:center;gap:8px;padding:8px;border:1px solid var(--border-subtle);border-radius:6px;cursor:pointer;background:var(--bg-secondary);">
            <input type="checkbox" id="mem-enabled" ${memoryEnabled ? 'checked' : ''}>
            <span style="font-weight:500;">启用记忆注入</span>
            <span style="font-size:12px;color:var(--text-tertiary);margin-left:auto;">开启后会注入历史执行记录，增加Token消耗</span>
          </label>
          <div id="mem-source-options" style="display:${memoryEnabled ? 'block' : 'none'};">
            <div style="font-size:12px;color:var(--text-secondary);margin:8px 0;">选择记忆来源：</div>
            ...
            <div style="padding:8px;border:1px solid var(--border-subtle);border-radius:6px;">
              ...
              <select class="select" id="mem-source-workflow" style="width:100%;margin-left:24px;" ${memSource.type !== 'workflow' ? 'disabled' : ''}>
                ...
                ${allWfs.map((w) => `<option value="${w.id}" ${memSource.workflowId === w.id ? 'selected' : ''}>${escapeHtml(w.name)}</option>`).join('')}
              </select>
            </div>
          </div>
        </div>
      ...
      `
        ...
        // Toggle memory source options visibility
        document.getElementById('mem-enabled')?.addEventListener('change', (e) => {
            const sourceOptions = document.getElementById('mem-source-options');
            if (sourceOptions) {
                sourceOptions.style.display = e.target.checked ? 'block' : 'none';
            }
        });
        // Enable/disable select based on radio
        document.querySelectorAll('[name="mem-source"]').forEach((radio) => {
            radio.addEventListener('change', () => {
                const selectEl = document.getElementById('mem-source-workflow');
                if (selectEl) {
                    selectEl.disabled = radio.value !== 'workflow';
                }
            });
        });
        document.getElementById('mem-save-btn')?.addEventListener('click', async () => {
            const memEnabled = document.getElementById('mem-enabled')?.checked || false;
            ...
            try {
                await API.updateWorkflow(workflow.id, { memoryEnabled: memEnabled, memorySource: newSource });
                workflow.memoryEnabled = memEnabled;
    ...
    async function openKnowledgeSettings(workflow) {
      ...
      `
        ...
        document.querySelectorAll('[name="kn-source"]').forEach((radio) => {
            radio.addEventListener('change', () => {
                const catSelect = document.getElementById('kn-category');
                if (catSelect)
                    catSelect.disabled = radio.value !== 'category';
    ...
    async function showRunDialog(id, wf, wfName) {
      ...
      `,
        ...
        document.getElementById('run-batch-mode')?.addEventListener('change', (e) => {
            ...
            if (singleEl)
                singleEl.style.display = e.target.checked ? 'none' : 'block';
            if (batchEl)
                batchEl.style.display = e.target.checked ? 'block' : 'none';
        });
    ...
    function createTaskForWorkflow(workflow) {
        if (!workflow)
            return;
        // Navigate to tasks page with workflow pre-selected
        Router.navigate('/tasks');
        // Wait for tasks page to load, then open create modal with workflow
        setTimeout(() => {
            if (typeof TaskCreate !== 'undefined') {
                TaskCreate.openWithWorkflow(workflow.id, workflow.name);
            }
        }, 300);
    }
    async function pauseWorkflow(id) {
        try {
            if (currentWorkflow && currentWorkflow.executionStatus === 'paused') {
    ...
    async function deleteWorkflow(id) {
        ...
        if (!await Modal.confirm('删除工作流', '确定删除此工作流？', false))
            return;
    ...
    function onWorkflowUpdate(payload) {
        ...
        if (currentWorkspaceFilter && payload.workspaceId && payload.workspaceId !== currentWorkspaceFilter)
            return;
    ...
    async function batchExecute() {
        ...
        const ids = Array.from(_selectedIds);
        if (!confirm(`确定执行选中的 ${ids.length} 个工作流？`))
            return;
        try {
    ...
    async function batchDelete() {
        ...
        if (!await Modal.confirm('批量删除', msg, false))
            return;
        const ids = Array.from(_selectedIds);
        try {
    ...
    function readOnlyZoom(factor) {
        ...
        if (g)
            g.setAttribute('transform', `translate(${_roPan.x},${_roPan.y}) scale(${_roZoom})`);
    }
    ...
    function updateReadonlyProgress(workflow) {
        ...
        if (progressBar) {
            ...
            if (barFill)
                barFill.style.background = failedCount > 0 ? 'var(--accent-red)' : 'var(--accent-green)';
        }
        // Update progress text
        const progressText = progressBar?.querySelector('span[style*="font-mono"]');
        if (progressText)
            progressText.textContent = `${completedCount}/${workNodes.length} (${progressPercent}%)`;
        // Update status badge
      ...
      `;
        ...
        layer.querySelectorAll('.ro-node').forEach((el) => {
            el.addEventListener('click', (e) => {
                if (e.target.closest('.port-in') || e.target.closest('.port-out'))
                    return;
