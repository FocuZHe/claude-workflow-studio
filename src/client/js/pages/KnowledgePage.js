...
window.KnowledgePage = (() => {
    ...
    el.innerHTML = `
      <div class="page-enter">
        <div class="page-header">
          <h1 class="page-title"><span class="page-icon">${Icon.svg('knowledge', 20)}</span> 知识库</h1>
  ...
  async function loadEntries(page) {
    ...
    if (page === 1) {
      listEl.innerHTML = LoadingState.render('加载中...');
    }

    try {
      const res = await API.searchKnowledge(_query, { category: _category, page, limit: PAGE_SIZE });
      if (seq !== _searchSeq) { _loadingMore = false; return; }
      const data = res.data || {};
      ...
      bindLoadMoreButton('load-more-kb', () => loadEntries(_currentPage + 1));
    } catch (e) {
  ...
  async function addEntry() {
    ...
    document.getElementById('kb-save-btn').addEventListener('click', async () => {
      ...
      if (!title || !content) {
        Toast.warning('标题和内容为必填');
        return;
      }

      try {
        await API.addKnowledge({ title, content, category, tags });
        Toast.success('已添加');
        Modal.close();
        await loadEntries();
      } catch (e) {
  ...
  function showExportDialog() {
    ...
    document.getElementById('export-confirm-btn')?.addEventListener('click', async () => {
      ...
      try {
        ...
        const url = API.exportKnowledge(format);
  ...
  function showImportDialog() {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (file.name.endsWith('.json')) {
            ...
            if (!Array.isArray(parsedEntries)) throw new Error('JSON 需包含 entries 数组');
    ...
    document.getElementById('import-confirm-btn')?.addEventListener('click', async () => {
      ...
      try {
        const res = await API.importKnowledge(parsedEntries);
        Toast.success(`成功导入 ${res.data?.imported || 0} 条知识`);
        Modal.close();
        await loadEntries();
      } catch (e) {
