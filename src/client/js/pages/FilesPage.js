...
window.FilesPage = (() => {
  ...
  function toggleWorkspacePanel() {
    ...
    if (panel) panel.style.display = _workspacesExpanded ? 'block' : 'none';
    if (arrow) {
  ...
  async function loadWorkspacesList() {
    try {
      // 同时获取活跃工作区、历史记录和全部工作流
      const [activeRes, infoRes, wfRes] = await Promise.allSettled([
        API.getWorkspaces(),
        API.getWorkspaceInfo(),
        API.getWorkflows({ limit: 9999 })
      ]);

      const activeList = activeRes.status === 'fulfilled'
        ? (Array.isArray(activeRes.value) ? activeRes.value : (Array.isArray(activeRes.value?.data) ? activeRes.value.data : []))
        : [];

      const historyList = infoRes.status === 'fulfilled'
        ? (infoRes.value?.data?.recentWorkspaces || []).map(h => ({ path: typeof h === 'string' ? h : h.path, name: typeof h === 'string' ? h.split(/[/\\\\]/).pop() : (h.name || h.path.split(/[/\\\\]/).pop()), fromHistory: true }))
        : [];

      // 解析工作流，与 WorkflowsPage 完全一致的方式
      let allWorkflows = [];
      if (wfRes.status === 'fulfilled' && wfRes.value) {
        const d = wfRes.value.data;
        allWorkflows = Array.isArray(d) ? d : (d?.items || []);
      }

      // 合并：活跃工作区优先，历史记录补充（按路径去重）
      const activePaths = new Set(activeList.map(w => w.path));
      const merged = [...activeList];
      for (const hw of historyList) {
        if (!activePaths.has(hw.path)) {
          merged.push(hw);
          activePaths.add(hw.path);
        }
      }

      // 构建活跃工作区 path→count 的映射（服务端已正确计算：全局 + 工作区专属）
      const activePathCountMap = new Map();
      for (const a of activeList) {
        activePathCountMap.set(a.path, { id: a.id, workflowCount: a.workflowCount || 0 });
      }

      for (const ws of merged) {
        const activeInfo = activePathCountMap.get(ws.path);
        if (activeInfo) {
          // 活跃工作区：直接使用服务端已计算好的准确数量
          ws.id = activeInfo.id;
          ws.workflowCount = activeInfo.workflowCount;
        } else {
          // 非活跃/历史工作区：无法获取准确数据，显示为未知
          ws.workflowCount = -1; // -1 表示"未知"
        }
      }

      _workspaces = merged;
      if (typeof Store !== 'undefined') Store.set('activeWorkspaces', _workspaces);
  ...
  function renderWorkspaceGrid(wsList) {
    ...
    if (countEl) countEl.textContent = `(${wsList.length} 个工作区)`;
  ...
  function renderWorkspaceCard(ws) {
    ...
    const workflowCount = ws.workflowCount === -1 ? '—' : (ws.workflowCount || 0);
  ...
  async function deactivateWorkspace(id) {
    if (!await Modal.confirm('停用工作区', '确定停用此工作区？停用后将从管理平台移除，工作区内的本地文件不会被删除，可随时重新激活。')) return;
    try {
  ...
  function updateNavbarWorkspaceCount() {
    ...
    if (el) el.textContent = _workspaces.length;
  }
  ...
  async function onFileSelect(path, type) {
    ...
    try {
      ...
      if (mdEditBtn) mdEditBtn.style.display = isMarkdown ? '' : 'none';
      if (mdPreviewBtn) mdPreviewBtn.style.display = isMarkdown ? '' : 'none';
      if (mdSplitBtn) mdSplitBtn.style.display = isMarkdown ? '' : 'none';
  ...
  async function showDirectoryPreview(dirPath) {
    ...
    try {
      ...
      const listEl = document.getElementById('dir-file-list');
      if (listEl) listEl.innerHTML = '<div style="color:var(--accent-red);">加载失败</div>';
    }
  }
      ...
      if (confirmBtn) {
        confirmBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            Modal.close();
            await loadTree();
          } catch (e) { Toast.error(e.message || '重命名失败'); }
        });
      }
      ...
      document.getElementById('delete-confirm-btn')?.addEventListener('click', async () => {
        try {
          ...
          if (currentPath === path) {
            ...
            _undoIndex = -1;
            const editor = document.getElementById('file-editor');
            if (editor) editor.innerHTML = '<div class="empty-state" style="height:100%;"><div class="empty-icon">▤</div><div class="empty-title">未选择文件</div></div>';
            const saveBtn = document.getElementById('save-file-btn');
            if (saveBtn) saveBtn.style.display = 'none';
            const undoBtn = document.getElementById('undo-btn');
            const redoBtn = document.getElementById('redo-btn');
  ...
  async function createFile() {
    ...
    if (confirmBtn) {
      confirmBtn.addEventListener('click', async () => {
        ...
        if (!p) { Toast.warning('请输入文件路径'); return; }
        try {
  ...
  async function createFolder() {
    ...
    if (confirmBtn) {
      confirmBtn.addEventListener('click', async () => {
        const p = input?.value.trim();
        if (!p) { Toast.warning('请输入文件夹路径'); return; }
        try {
          await API.mkdir(p);
          Toast.success('文件夹已创建');
          Modal.close();
  ...
  async function loadWorkspaceInfo() {
    try {
      ...
      const display = document.getElementById('workspace-path-display');
      if (display) display.textContent = '无法获取工作区信息';
    }
  }
  ...
  async function showImportWorkflowsDialog(workspacePath) {
    try {
      ...
      const importable = allWorkflows.filter(wf => {
        if (!wf.folderPath) return false; // global already
  ...
  async function navigateBack() {
    ...
    if (_isReadOnlyFile || (currentPath && !_currentDirPath)) {
      ...
      if (parent) {
        ...
        if (pathDisplay) pathDisplay.textContent = '请选择要查看的文件';
        if (saveBtn) saveBtn.style.display = 'none';
        if (backBtn) backBtn.style.display = 'none';
        if (undoBtn) undoBtn.style.display = 'none';
        if (redoBtn) redoBtn.style.display = 'none';
  ...
  function pushUndoSnapshot() {
    ...
    if (_undoHistory.length > 0 && _undoHistory[_undoIndex] === content) return;
  ...
  function performRedo() {
    if (_undoIndex >= _undoHistory.length - 1) return;
    _undoIndex++;
    const textarea = document.getElementById('file-content');
    if (textarea) {
      textarea.value = _undoHistory[_undoIndex];
  ...
  function updateUndoButtons() {
    ...
    if (redoBtn) redoBtn.disabled = _undoIndex >= _undoHistory.length - 1;
  }

  // ── Undo Cache Persistence ──

  function debouncedSaveUndoCache() {
    if (_undoCacheSaveTimer) clearTimeout(_undoCacheSaveTimer);
  ...
  async function saveUndoCacheToBackend() {
    if (!currentPath || _undoHistory.length === 0) return;
    try {
  ...
  async function loadUndoCache() {
    ...
    try {
      ...
      if (res?.data && Array.isArray(res.data.history) && res.data.history.length > 0) {
        ...
        // Clamp index
        if (_undoIndex < 0) _undoIndex = 0;
        if (_undoIndex >= _undoHistory.length) _undoIndex = _undoHistory.length - 1;
      } else {
  ...
  function switchMdMode(mode) {
    ...
    if (editBtn) editBtn.className = 'btn btn-sm ' + (mode === 'edit' ? 'btn-primary' : 'btn-secondary');
    if (previewBtn) previewBtn.className = 'btn btn-sm ' + (mode === 'preview' ? 'btn-primary' : 'btn-secondary');
    if (splitBtn) splitBtn.className = 'btn btn-sm ' + (mode === 'split' ? 'btn-primary' : 'btn-secondary');
    ...
    if (mode === 'edit') {
      ...
      if (newTa) {
        newTa.addEventListener('input', () => {
          ...
          if (preview) preview.innerHTML = renderMarkdown(newTa.value);
        });
        newTa.addEventListener('keydown', handleEditorKeydown);
      }
    }
  ...
  function openImportDialog() {
    ...
    sourcePathInput?.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') byPathBtn?.click();
    });
  }
  ...
  function cleanup() {
    ...
    if (_undoCacheSaveTimer) { clearTimeout(_undoCacheSaveTimer); _undoCacheSaveTimer = null; }
