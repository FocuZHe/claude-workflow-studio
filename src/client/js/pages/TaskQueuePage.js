...
window.TaskQueuePage = (() => {
  ...
  function getFiltered() {
    return queues.filter(q => {
      ...
      if (filterWorkflow && q.workflowId !== filterWorkflow) return false;
      return true;
    });
  }
  ...
  function bindCreateEvents(queueItems) {
    ...
    function bindItemEvents() {
      document.querySelectorAll('.queue-item-text').forEach(el => {
        el.addEventListener('input', (e) => {
          ...
          if (queueItems[idx]) queueItems[idx].input = e.target.value;
        });
      });
    ...
    document.getElementById('queue-submit-btn')?.addEventListener('click', async () => {
      ...
      document.querySelectorAll('.queue-item-text').forEach(el => {
        ...
        if (queueItems[idx]) queueItems[idx].input = el.value;
      });
      ...
      if (!name) { Toast.warning('队列名称为必填项'); return; }
      if (!workflowId) { Toast.warning('请选择工作流'); return; }

      const validItems = queueItems.filter(i => i.input.trim());
      if (validItems.length === 0) { Toast.warning('至少需要一个有效任务'); return; }
  ...
  async function resumeQueue(id) {
    ...
    try {
      ...
      await loadQueues();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.textContent = '⏵'; } }
  }
  ...
  async function deleteQueue(id) {
    ...
    try {
      ...
      await loadQueues();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('delete', 14); } }
  }
