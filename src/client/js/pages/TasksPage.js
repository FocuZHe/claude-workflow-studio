// ═══════════════════════════════════════════════
// Tasks Page — Integrated tasks + queue management
// ═══════════════════════════════════════════════
...
window.TasksPage = (() => {
  ...
  async function render() {
    ...
    document.getElementById('task-search').addEventListener('compositionend', (e) => {
      clearTimeout(_searchTimer);
      _searchTimer = setTimeout(() => { tasks = []; _currentPage = 1; _totalItems = 0; loadTasks(1); }, 300);
    });
    document.getElementById('filter-task-type').addEventListener('change', (e) => {
      filterType = e.target.value;
      tasks = []; _currentPage = 1; _totalItems = 0;
      loadTasks(1);
    });
  ...
  async function loadAgents() {
    ...
    } catch (e) { Toast.error('加载Agent列表失败'); }
  }
  ...
  function getFiltered() {
    ...
    if (filterType !== 'task') {
      queues.forEach(q => {
        ...
        if (filterStatus && q.status !== filterStatus) return;
  ...
  function renderList() {
    ...
    container.innerHTML = `<div class="stagger">${filtered.map(item => {
      ...
      if (item._type === 'queue') return renderQueueCard(item);
  ...
  async function executeTask(id) {
    ...
    try {
      ...
      await loadTasks();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('play', 14); } }
  }
  ...
  async function pauseTask(id) {
    ...
    try {
      ...
      await loadTasks();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('pause', 14); } }
  }
  ...
  async function cancelTask(id) {
    ...
    try {
      ...
      await loadTasks();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('stop', 14); } }
  }

  async function deleteTask(id) {
    if (!await Modal.confirm('删除任务', '确定删除此任务？')) return;
    const card = document.querySelector(`.task-card[data-id="${id}"]`);
    ...
    try {
      ...
      await loadTasks();
      renderList();
    } catch (e) { Toast.error(e.message); if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('close', 14); } }
  }
  ...
  async function batchDelete() {
    ...
    if (!await Modal.confirm('批量删除', `确定删除选中的 ${_selectedIds.size} 个项目？此操作不可撤销。`)) return;
    ...
    try {
      ...
      const promises = [];
      if (taskIds.length > 0) promises.push(API.deleteTasksBatch(taskIds));
      if (queueIds.length > 0) promises.push(API.deleteTaskQueuesBatch(queueIds));
