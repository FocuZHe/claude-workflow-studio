...
window.AgentDetail = (() => {
    ...
    async function loadSkillsForEdit(currentSkillNames) {
      ...
      `;
            ...
            container.innerHTML = '<span style="color:var(--text-muted);">加载失败</span>';
        }
    }
    ...
    function renderForm(agent) {
      ...
      <div class="form-group">
        ...
        <select class="select" id="agent-role">
          ${[['developer', '开发者'], ['reviewer', '审查员'], ['tester', '测试员'], ['planner', '规划师'], ['debugger', '调试员'], ['documenter', '文档员'], ['custom', '自定义']].map(([v, l]) => `<option value="${v}" ${agent.role === v ? 'selected' : ''}>${l}</option>`).join('')}
        </select>
      </div>
      <div class="form-group">
        <label class="form-label">状态</label>
        <select class="select" id="agent-status">
          ${[['idle', '空闲'], ['busy', '忙碌'], ['error', '错误'], ['offline', '离线']].map(([v, l]) => `<option value="${v}" ${agent.status === v ? 'selected' : ''}>${l}</option>`).join('')}
        </select>
      </div>
      <div class="form-group">
