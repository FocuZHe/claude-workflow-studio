// ═══════════════════════════════════════════════
// Analytics Page — Workflow Statistics & Timeline
// ═══════════════════════════════════════════════
...
window.AnalyticsPage = (() => {
  ...
  function toggleItem(key) {
    ...
    if (cb) cb.checked = _selectedIds.has(key);
    const selectAll = document.getElementById('analytics-select-all');
    if (selectAll) selectAll.checked = _selectedIds.size === _timeline.length && _timeline.length > 0;
  }
  ...
  async function batchDelete() {
    ...
    if (!await Modal.confirm('批量删除执行记录', `确定删除 ${count} 条执行记录？删除后统计数据将重新计算。`)) return;
    ...
    try {
      ...
      const btn = document.getElementById('analytics-select-btn');
      const deleteBtn = document.getElementById('analytics-batch-delete-btn');
      if (btn) { btn.textContent = '批量选择'; btn.style.borderColor = ''; }
