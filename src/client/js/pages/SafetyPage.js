...
window.SafetyPage = (() => {
  ...
  function updateStatsUI() {
    ...
    if (threatsEl) threatsEl.textContent = stats.todayThreats || 0;
    if (rulesEl) rulesEl.textContent = stats.activeRules || 0;
    if (blockedEl) blockedEl.textContent = stats.blockedRequests || 0;
  }
    ...
    try {
      ...
      tableContainer.querySelectorAll('.rule-delete-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          if (!await Modal.confirm('删除规则', '确定删除此安全规则？此操作不可撤销。')) return;
          try {
