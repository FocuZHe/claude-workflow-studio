...
window.DashboardPage = (() => {
  async function render() {
    ...
    el.innerHTML = `
      <div class="page-enter">
        <div class="page-header">
          <h1 class="page-title"><span class="page-icon">${Icon.svg('dashboard', 20)}</span> 控制面板</h1>
    ...
    setTimeout(() => {
      ...
      if (resRefreshBtn) resRefreshBtn.addEventListener('click', () => loadResources());
    }, 0);
  ...
  function startUptimeClock(statusEl) {
    ...
    _uptimeInterval = setInterval(() => {
      if (!statusEl || !statusEl.parentNode) { clearInterval(_uptimeInterval); return; }
      const uptime = Math.floor((Date.now() - _serverStartTime) / 1000);
      let extra = '';
      if (!_cliOk) extra += `<span style="color:var(--accent-amber);margin-left:8px;font-size:11px;">CLI ✗</span>`;
      if (!_sdkOk) extra += `<span style="color:var(--accent-amber);margin-left:4px;font-size:11px;">SDK ✗</span>`;
      statusEl.innerHTML = `
  ...
  async function loadStats() {
    try {
      ...
      if (agentStat) agentStat.textContent = agents.length;
      if (workflowStat) workflowStat.textContent = workflows.filter(w => w.status === 'running').length;
      if (taskStat) taskStat.textContent = tasks.filter(t => t.status === 'pending').length;
      if (chatStat) chatStat.textContent = chatCount;
      if (termStat) termStat.textContent = termCount;
