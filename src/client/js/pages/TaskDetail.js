...
window.TaskDetail = (() => {
  ...
  function open(task, agents = []) {
    ...
    // 监听 WebSocket 事件
    _wsUnsubs.push(WS.on('task.progress', onProgress));
    _wsUnsubs.push(WS.on('task.completed', onCompleted));
    _wsUnsubs.push(WS.on('task.failed', onFailed));
  }
  ...
  function onProgress(payload) {
    if (payload.taskId !== _currentTaskId) return;
    if (payload.message) {
  ...
  async function refreshDetail() {
    ...
    try {
      const res = await API.getTask(_currentTaskId);
      const task = res.data;
      const body = document.querySelector('.modal-body');
      if (body) {
  ...
  function renderDetail(task, agent) {
    return `
      ...
      ${LogViewer.render(task.logs || [], 'task-detail-logs')}
