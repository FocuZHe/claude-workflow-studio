...
window.AgentsPage = (() => {
  ...
  async function render() {
    ...
    document.getElementById('agent-search').addEventListener('compositionend', (e) => {
      clearTimeout(_searchTimer);
      _searchTimer = setTimeout(() => {
        searchQuery = (e.target.value || e.data || '').toLowerCase();
        agents = [];
        _currentPage = 1;
        _totalItems = 0;
        loadAgents(1);
  ...
  async function loadAgents(page) {
    ...
    try {
      ...
      if (ver === _pageVer) _loadingMore = false;
    }
  }
  ...
  function openCreateModal() {
    AgentCreate.open(async (data) => {
      ...
      try {
        ...
        if (btn) { btn.disabled = false; btn.textContent = '+ 创建智能体'; }
      }
    });
  }
  ...
  async function deleteAgent(id) {
    if (!await Modal.confirm('删除智能体', '确定删除此智能体？')) return;
    ...
    try {
      ...
      Toast.error(e.message);
      if (btn) { btn.disabled = false; btn.innerHTML = Icon.svg('close', 14); }
    }
  }
  ...
  async function batchDelete() {
    ...
    if (!await Modal.confirm('批量删除', `确定删除选中的 ${_selectedIds.size} 个智能体？此操作不可撤销。`)) return;
