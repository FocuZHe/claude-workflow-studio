...
window.TerminalPage = (() => {
  ...
  async function render() {
    ...
    el.innerHTML = `
      <div class="page-enter" style="display:flex;flex-direction:column;">
        <div class="page-header" style="flex-shrink:0;">
          <h1 class="page-title"><span class="page-icon">⌨️</span> 终端</h1>
    ...
    const exitHandler = (payload) => {
      ...
      if (session) {
        ...
        if (session.id === activeSessionId) renderSessionList();
        saveState();
      }
    };
  ...
  function renderSessionList(list) {
    ...
    container.querySelectorAll('.term-session-item').forEach(el => {
      ...
      el.addEventListener('mouseenter', () => {
        if (el.dataset.id !== activeSessionId) el.style.background = 'var(--bg-secondary)';
        const delBtn = el.querySelector('.term-session-delete');
        if (delBtn) delBtn.style.opacity = '1';
      });
      el.addEventListener('mouseleave', () => {
        if (el.dataset.id !== activeSessionId) el.style.background = '';
        const delBtn = el.querySelector('.term-session-delete');
    ...
    container.querySelectorAll('.term-session-title').forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        const finish = (confirmed) => {
          ...
          if (confirmed) session.title = val;
  ...
  function switchSession(id) {
    ...
    // Clear container and attach/create terminal
    container.innerHTML = '';
    const entry = XtermTerminal.create(id, container);
  ...
  async function closeSession(id) {
    ...
    if (activeSessionId) {
      ...
      const container = document.getElementById('term-xterm-container');
      if (container) container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);">无活动会话。点击"+ 新建会话"创建一个。</div>';
    }
    saveState();
  }
