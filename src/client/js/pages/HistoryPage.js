...
window.HistoryPage = (() => {
  ...
  async function loadHistory(page) {
    ...
    try {
      ...
      if (currentFilter.search) params.workflowName = currentFilter.search;
  ...
  async function deleteSingleHistory(runId) {
    if (!await Modal.confirm('删除记录', '确定删除此执行记录？此操作不可撤销。')) return;
    try {
  ...
  async function batchDelete() {
    ...
    if (!await Modal.confirm('批量删除', `确定删除选中的 ${_selectedIds.size} 条执行记录？此操作不可撤销。`)) return;
