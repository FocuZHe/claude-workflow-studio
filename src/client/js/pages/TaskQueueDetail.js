...
window.TaskQueueDetail = (() => {
    ...
    async function loadAndRender() {
        if (!_currentQueueId)
            return;
        try {
            const res = await window.API.getTaskQueue(_currentQueueId);
            const queue = res.data;
            showDetail(queue);
        }
        catch (e) {
            window.Toast.error('加载队列详情失败');
        }
    }
    ...
    function renderDetail(queue, items, total, completed, failed, percent) {
      ...
      <div style="margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
          ${window.StatusBadge.render(queue.status)}
          ${queue.status === 'running' ? '<span class="running-indicator"></span>' : ''}
          ${queue.workflowName ? `<span style="color:var(--text-tertiary);font-size:12px;">${window.Icon.svg('workflow', 14)} ${escapeHtml(queue.workflowName)}</span>` : ''}
        </div>
    ...
    function renderItemRow(item, idx) {
      ...
      <tr>
        ...
        <td>
          ...
          ${isWaiting && item.nodeName ? `<div style="font-size:11px;color:var(--accent-purple);margin-top:4px;">${window.Icon.svg('spinner', 14)} 等待人工响应: ${escapeHtml(item.nodeName)}</div>` : ''}
        ...
        <td>
          <div style="display:flex;align-items:center;gap:4px;">
            ...
            ${isWaiting ? `<span style="color:var(--accent-purple);animation:pulse 1.5s infinite;">${window.Icon.svg('spinner', 14)}</span>` : ''}
    ...
    function bindActions(queue) {
        ...
        document.querySelector('.tqd-btn-start')?.addEventListener('click', async () => {
            ...
            try {
                await window.API.startTaskQueue(id);
                window.Toast.success('队列已开始');
                await loadAndRender();
            }
            catch (e) {
        ...
        document.querySelector('.tqd-btn-pause')?.addEventListener('click', async () => {
            ...
            try {
                await window.API.pauseTaskQueue(id);
                window.Toast.success('队列已暂停');
                await loadAndRender();
            }
            catch (e) {
                window.Toast.error(e.message);
        ...
        document.querySelector('.tqd-btn-resume')?.addEventListener('click', async () => {
            ...
            try {
                await window.API.resumeTaskQueue(id);
                window.Toast.success('队列已继续');
                await loadAndRender();
            }
            catch (e) {
                window.Toast.error(e.message);
        ...
        document.querySelector('.tqd-btn-cancel')?.addEventListener('click', async () => {
            if (!await window.Modal.confirm('取消队列', '确定取消此队列？正在执行的任务将被中断。'))
                return;
            ...
            try {
                await window.API.cancelTaskQueue(id);
                window.Toast.success('队列已取消');
                await loadAndRender();
            }
            catch (e) {
                window.Toast.error(e.message);
        ...
        document.querySelector('.tqd-btn-delete')?.addEventListener('click', async () => {
            if (!await window.Modal.confirm('删除队列', '确定删除此队列？此操作不可撤销。'))
                return;
            ...
            try {
                await window.API.deleteTaskQueue(id);
                window.Toast.success('队列已删除');
                cleanup();
                window.Modal.close();
            }
            catch (e) {
    ...
    function subscribeWs() {
        ...
        events.forEach(evt => {
            _wsUnsubs.push(window.WS.on(evt, (payload) => {
