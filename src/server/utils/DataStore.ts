...
class DataStore {
  ...
  /**
   * @param filePath - JSON 文件路径（如 data/workflows.json）
   ...
   * 异步保存 — 保留以兼容旧调用方。
   */
  async saveAsync(data: any[]): Promise<void> {
    this.save(data);
  }
      ...
      try {
        if (fs.existsSync(this.sqlitePath)) {
          ...
          this._data = res.length > 0
            ? res[0].values.map((row: any[]) => JSON.parse(row[0]))
            : [];
          this._useSqlite = true;
          return;
        }
