...
app.use(cors({
  origin(origin, cb) {
    ...
    if (!origin || corsOrigin.indexOf(origin) !== -1) return cb(null, true);
...
let cliVersionPromise: Promise<boolean> | null = null;
...
function gracefulShutdown(signal: string): void {
  ...
  try { TerminalService.killAll(); } catch (e) { /* ignore */ }
  try { logger.close(); } catch (e) { /* ignore */ }
  server.close(() => {
