...
class SdkService extends EventEmitter {
   ...
   */
  private _getCircuitBreaker(taskId: string): any {
  ...
  _saveCache(): void {
    try {
      ...
      fs.writeFileSync(
        this._cacheFilePath,
        JSON.stringify([...this._completedTasks]),
        'utf-8'
      );
    } catch (e) {
   ...
   */
  _parseJsonSafely(rawText: string): any {
    try {
      // 提取可能的JSON边界
   ...
   * 计算工具调用深度（用于级联追踪）
   */
  _calculateDepth(taskId: string, parentToolUseId: string): number {
    const tree = this._callingTrees.get(taskId);
    if (!tree) return 0;
   ...
   * @param taskId 主任务ID
   */
  _cleanupCallingTree(taskId: string): void {
   ...
   */
  async _createWorktree(agentId: string, workspaceRoot: string): Promise<string> {
   ...
   */
  async _forcePruneWorktree(agentId: string, workspaceRoot: string): Promise<void> {
    const worktreePath = path.join(workspaceRoot, '.worktrees', agentId);
    const branchName = `agent-${agentId}`;
   ...
   */
  async _cleanupAllWorktrees(workspaceRoot: string): Promise<void> {
   ...
   */
  async _getWorktreeSummary(worktreePath: string): Promise<string> {
    try {
      ...
      let summary = '';
      if (diff.trim()) summary += `修改的文件:\n${diff}\n`;
      if (newFiles.trim()) summary += `新增的文件:\n${newFiles}\n`;
   ...
   */
  async _createWorktreeWithLock(agentId: string, workspaceRoot: string): Promise<string> {
   ...
   */
  async _cleanupWorktreeWithLock(agentId: string, workspaceRoot: string): Promise<void> {
   ...
   * @returns 最终结果
   */
  async executeAutonomousDecisionFlow(
    taskId: string,
    initialPrompt: string,
    workspaceRoot: string,
    maxAttempts: number = 3,
    evaluatorModel: string = 'haiku'
  ): Promise<string> {
...
`;
        ...
        // 使用快速模型进行自主评估
        const evalRaw = await this._executeWithClaudeSdk(
          `${taskId}_evaluator`,
          null,
          evalPrompt,
          { folderPath: worktreePath, model: evaluatorModel }
        );
   ...
   * @returns 汇聚后的结果
   */
  async executeForkJoinFlow(
    tasks: Array<{ id: string; description: string; model?: string }>,
    mergePrompt: string,
    workspaceRoot: string
  ): Promise<string> {
...
`;
      ...
      const finalResult = await this._executeWithClaudeSdk(
        mergeTaskId,
        null,
        joinPrompt,
      ...
      await Promise.all(
        tasks.map(task =>
          this._cleanupWorktreeWithLock(task.id, workspaceRoot)
        )
      );
    }
  }
   ...
   * @returns 最终结果
   */
  async executeHumanInTheLoopFlow(
    taskId: string,
    initialPrompt: string,
    workspaceRoot: string,
    approvalId?: string
   ...
   * 处理人工审批结果（由前端调用）
   */
  handleHitlApproval(approvalId: string, approved: boolean, feedback?: string): void {
   ...
   * 捕获故障现场
   */
  _captureCrashScene(taskId: string, logFile: string, error: any): void {
    try {
      const crashReport = `
========== 故障现场 ==========
...
如果遇到错误：记录到文件并继续，不要静默忽略。`;
   ...
   * 处理审批结果（由前端调用）
   */
  handleApprovalResult(approvalId: string, approved: boolean, feedback?: string): void {
    const approval = this._pendingApprovals.get(approvalId);
    if (!approval) {
        ...
        if (toolBlocks.length > 0) {
          ...
          for (const block of toolBlocks) {
            ...
            if (block.name && block.name.startsWith('Agent_') && this._checkpointCallback) {
              ...
              if (nodeInfo) {
                this._checkpointCallback(nodeId, nodeInfo.label || nodeId, (toolResult as any)?.content || '');
              }
            }
          }
    ...
    if (backgroundTasks.length > 0) {
      ...
      for (let i = 0; i < bgResults.length; i++) {
        ...
        if (result.status === 'fulfilled') {
          const resultValue = String((result as any).value || '');
          fullText += `\n\n[后台子Agent "${task.description}" 完成]\n${resultValue.substring(0, 2000)}`;
          this._broadcastChunk(taskId, agentId, `\n[后台子Agent "${task.description}" 完成]\n`, false);
        } else {
          fullText += `\n\n[后台子Agent "${task.description}" 失败: ${(result as any).reason?.message}]\n`;
        }
      }
    }
...
- 结论必须准确简洁`;
    ...
    try {
      ...
      const timeoutPromise = new Promise<string>((_, reject) =>
        setTimeout(() => reject(new Error('Sub-agent timed out')), SUBAGENT_TIMEOUT)
      );
...
5. 如果需要进一步分工，可以使用 Agent 工具委派子任务（最多 ${this._maxAgentDepth - depth} 层嵌套）。`;
    ...
    while (true) {
      ...
      // max_tokens — continue
      messages.push({ role: 'assistant', content } as any);
      messages.push({ role: 'user', content: [{ type: 'text', text: 'Please continue.' }] } as any);
    }
   ...
   */
  private _bufferedBroadcast(taskId: string, chunk: string, done: boolean) {
