...
export class WorkflowService {
  ...
  static async _executeWithOrchestrator(
    ...
    if (WorkflowService._claudeService || (global as any).__sdkService) {
        ...
        try {
          ...
          if (node.type === 'start') {
            ...
            if (claudeService && typeof claudeService.execute === 'function') {
              output = await claudeService.execute(
                runId,
                node.agentId || node.id,
                prompt,
                node.config || {}
              );
            } else {
  ...
  static async step(workflowId: string, nodeId: string): Promise<StepResult> {
    ...
    try {
      const output = await claudeService.execute(runId, null, nodeInput, {
        systemPrompt,
        model: (() => { try { return ApiKeyService.resolveModel('sonnet'); } catch (_) { return 'sonnet'; } })(),
