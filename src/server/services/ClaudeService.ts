...
export class ClaudeService extends EventEmitter {
  ...
  private activeProcesses: Map<string, { process: ChildProcess; startedAt: Date }>;
  private _taskWorkflowMap: Map<string, string>;
  private _taskMetaMap: Map<string, any>;
  ...
  private async _executeInternal(
    ...
    const workingDir =
      folderPath || config.workingDir || configModule.workspaceRoot || process.cwd();
    const timeoutMs =
      config.timeoutMs !== undefined ? config.timeoutMs : 30 * 60 * 1000; // 30 minutes default
...
`;
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      // 创建命令转换包装脚本
      try {
        fs.writeFileSync(path.join(workingDir, 'bash-wrapper.sh'), bashWrapper, { mode: 0o755 });
      } catch (_) {}
      ...
      let memMonitor: NodeJS.Timeout | null = null;
        ...
        memMonitor = setInterval(() => {
          try {
            ...
            if (process.platform === 'win32') {
              ...
              const out = execSync(
                `powershell -Command "(Get-Process -Id ${procPid}).WorkingSet64"`,
              ...
              if (!isNaN(rssKb)) memBytes = rssKb * 1024;
            }
            if (!isNaN(memBytes)) {
      ...
      const cleanupExecDir = () => {
        ...
        try {
          fs.unlinkSync(path.join(workingDir, 'CLAUDE.md'));
        } catch (_) {}
      };
   ...
   * 广播流式输出块
   */
  private _broadcastChunk(taskId: string, agentId: string | null, chunk: string, isComplete: boolean): void {
