...
interface SharedPool {
  ...
  notes: Array<{ workflowId: string; [key: string]: any }>;
  recentOutputs: Record<string, any>;
  lastUpdated?: string;
}
...
class MemoryService {
  ...
  static _cache: Map<string, CacheEntry> = new Map();
  static _CACHE_TTL: number = 5000; // 5 seconds
  ...
  static _getMemoryPath(workflowId: string): string {
  ...
  static getMemory(workflowId: string): string {
    try {
      // Check cache first
  ...
  static updateMemory(workflowId: string, content: string): boolean {
    try {
      const filePath = MemoryService._getMemoryPath(workflowId);
  ...
  static appendMemory(workflowId: string, entry: string): boolean {
  ...
  static appendMemoryWithTag(workflowId: string, entry: string, tag: string = ''): boolean {
  ...
  static extractAgentMemory(output: string): string[] {
    if (!output) return [];
  ...
  static injectMemoryFiltered(workflowId: string, taskInput: string): string {
   ...
   */
  static _isDuplicate(existing: string, newEntry: string): boolean {
    try {
  ...
  static _compressMemory(content: string): string {
  ...
  static getSharedPool(): SharedPool {
    try {
      if (!MemoryService._baseDir) return { variables: {}, notes: [], recentOutputs: {} };
  ...
  static updateSharedPool(data: Partial<SharedPool>): boolean {
    try {
      if (!MemoryService._baseDir) return false;
      const filePath = path.join(MemoryService._baseDir, 'shared', 'pool.json');
   ...
   */
  static extractSummary(output: string): string {
    if (!output) return '';
    const str = String(output);
  ...
  static injectMemory(workflowId: string): string {
    const memory = MemoryService.getMemory(workflowId);
    if (!memory || memory.trim().length === 0) return '';
    return `\n\n[Workflow Memory - Previous Sessions]\n${memory.substring(0, 10000)}\n`;
  }

  static listMemories(): MemoryListItem[] {
    try {
  ...
  static deleteMemory(workflowId: string): boolean {
    try {
      const filePath = MemoryService._getMemoryPath(workflowId);
   ...
   */
  static cleanSharedPool(workflowId: string): boolean {
    try {
   ...
   */
  static archiveMemory(_workflowId: string): boolean {
  ...
  static getMemoryLegacy(_agentId: string, workflowId: string): string {
    return MemoryService.getMemory(workflowId);
  }
  static appendMemoryLegacy(_agentId: string, workflowId: string, entry: string): boolean {
    return MemoryService.appendMemory(workflowId, entry);
  }
  static injectMemoryLegacy(_agentId: string, workflowId: string): string {
