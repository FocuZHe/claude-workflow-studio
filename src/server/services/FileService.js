 ...
 */
class FileService {
  /** @type {string|null} Runtime override for workspace root */
  static runtimeWorkspaceRoot = null;

  /** @type {Map<string, { excludedPaths: string[] }>} 按 runId 追踪的活跃执行 */
   ...
   * @param {string} newRoot - Absolute or relative path to use as workspace root
   * @returns {{ path: string, loadedState: Object|null }} The resolved path and loaded state
   */
  static setWorkspaceRoot(newRoot) {
    ...
    try {
      const ArtifactIndexService = require('./ArtifactIndexService');
      ...
      ArtifactIndexService.stopWatching(); // 停止旧监听
      ArtifactIndexService.init(resolved); // init 内部会调用 startWatching
      CheckpointService.init(resolved);
      ReportService.init(resolved);
      MemoryService.init(resolved);
      SnapshotService.init(resolved);
      TagService.init(resolved);
      KnowledgeService.init(resolved);
    } catch (e) { /* ignore */ }
   ...
   * @param {string} runId - 执行的唯一标识
   * @param {string[]} excludedPaths - 需要排除的绝对路径
   */
  ...
  static createDirectory(relativePath) {
    ...
    if (path.isAbsolute(normalized) || /^[A-Za-z]:/.test(normalized)) {
      ...
      fullPath = FileService.resolvePath(relativePath);
    }
  ...
  static createWorkspace(name, template, parentPath) {
    ...
    if (parentPath && (path.isAbsolute(normalizedParent) || /^[A-Za-z]:/.test(normalizedParent))) {
      ...
      parentDir = FileService.resolvePath(parentPath);
    } else {
      parentDir = FileService.getWorkspaceRoot();
      if (!parentDir) {
   ...
   * @returns {{ currentPath: string, parentPath: string|null, directories: Array }}
   */
