...
const CHAT_WORKSPACE: string = path.join(require('os').tmpdir(), 'claude-chat-workspace');

function ensureChatWorkspace(): void {
...
const DEFAULT_SYSTEM_PROMPT: string = `你是一个智能助手，专门为用户提供问题解答和方案建议。
...
你可以使用的工具：
- Bash：执行 shell 命令（包括 curl 请求网络）
- Read/Write/Edit：文件操作
- Grep/Glob：搜索文件内容

当你需要搜索网络信息时，使用 Bash 工具执行 curl 命令来获取数据。例如：
- 查天气：curl -s "https://wttr.in/深圳?format=3"
- 搜索信息：curl -s "https://api.duckduckgo.com/?q=查询内容&format=json"
- 获取网页：curl -s "https://网页URL"

你应该做的：
1. 当用户需要最新信息时，主动使用 Bash + curl 搜索网络
2. 可以阅读和分析文件，理解代码和文档内容
3. 为用户提供详尽、可靠的解决方案和专业建议
4. 用中文回复用户，语气友好专业，条理清晰`;

const VALID_MODELS: readonly string[] = ['opus', 'sonnet', 'haiku'] as const;
...
class ChatService {
  ...
  static readonly SLASH_COMMANDS: SlashCommand[] = [
   ...
   */
  static createSession(data: Partial<ChatSession> = {}): ChatSession {
   ...
   * Get all sessions
   */
  static getSessions(filters: ChatSessionFilters = {}): PaginatedResult<ChatSession> {
   ...
   * Get session by ID
   */
  static getSession(id: string): ChatSession {
    const session: ChatSession | null = ChatSessionModel.findById(id);
    if (!session) {
  ...
  static updateSession(id: string, data: Partial<ChatSession>): ChatSession {
    const session: ChatSession | null = ChatSessionModel.findById(id);
    if (!session) {
      throw new AppError('NOT_FOUND', `Chat session '${id}' not found`, 404);
    }
   ...
   */
  static archiveSession(id: string): ChatSession {
    const session: ChatSession | null = ChatSessionModel.findById(id);
    if (!session) {
      throw new AppError('NOT_FOUND', `Chat session '${id}' not found`, 404);
    }
   ...
   */
  static async sendMessage(sessionId: string, content: string): Promise<SendMessageResult> {
    const session: ChatSession | null = ChatSessionModel.findById(sessionId);
    if (!session) {
    ...
    if (slashMatch) {
      ...
      if (knownCommand) {
        // 添加用户消息
        const userMessage: ChatMessage = ChatSessionModel.addMessage(sessionId, {
          role: 'user',
          content
        });
        ...
        // 添加助手消息（命令结果）
        const assistantMessage: ChatMessage = ChatSessionModel.addMessage(sessionId, {
          role: 'assistant',
    ...
    const response: string = await this._callClaude(sessionId, content, session.model, session.systemPrompt);

    // Detect actionable content in the response
    const actions: ChatAction[] = this._detectActions(response);
    ...
    if (actions.length > 0 && this._broadcastService) {
      ...
      const refreshedSession: ChatSession | null = ChatSessionModel.findById(sessionId);
      if (refreshedSession &&
        ...
        this._generateTitle(sessionId, refreshedSession.messages.slice(0, 4))
          .catch((err: Error) => logger.warn('Auto-generate title failed', { sessionId, error: err.message }));
      }
   ...
   */
  static _detectActions(response: string): ChatAction[] {
   ...
   */
  static addSystemMessage(sessionId: string, content: string): ChatMessage {
   ...
   */
  static async executeConfirmedAction(sessionId: string, action: { type: string; path?: string; content?: string; command?: string }): Promise<any> {
    const session: ChatSession | null = ChatSessionModel.findById(sessionId);
    if (!session) {
      throw new AppError('NOT_FOUND', `Chat session '${sessionId}' not found`, 404);
    }
    ...
    const workspaceRoot: string | null = FileService.getWorkspaceRoot();
    ...
    if (action.type === 'write') {
      ...
      const filePath = path.join(workspaceRoot, action.path);
   ...
   */
  static executeSlashCommand(sessionId: string, command: string, args: string): any {
    const session: ChatSession | null = ChatSessionModel.findById(sessionId);
    if (!session) {
      throw new AppError('NOT_FOUND', `Chat session '${sessionId}' not found`, 404);
    }
    ...
    switch (command) {
      ...
      case '/config': {
        if (!args) return { type: 'config', current: { model: VALID_MODELS.includes(session.model) ? session.model : 'haiku', systemPrompt: session.systemPrompt, status: session.status } };
        const cfgParts = args.split(/\s+/);
        const cfgKey = cfgParts[0];
        const cfgVal = cfgParts.slice(1).join(' ');
        if (!cfgVal) return { type: 'config', key: cfgKey, value: (session as any)[cfgKey] };
      ...
      case '/export': {
        const exportText = session.messages.map((m: ChatMessage) =>
   ...
   */
  static _buildPrompt(messages: ChatMessage[], currentInput: string): string {
  ...
  static _sdkSessionIds: Map<string, string> = new Map();

  /**
   * 使用Claude Agent SDK的query函数调用Claude
   * 支持会话复用（resume），避免每次调用都启动新子进程
   */
  static async _callClaude(sessionId: string, prompt: string, model: string, systemPrompt: string): Promise<string> {
    const ApiKeyService = require('./ApiKeyService');
    const { query } = require('@anthropic-ai/claude-agent-sdk');

    const finalSystemPrompt: string = systemPrompt || DEFAULT_SYSTEM_PROMPT;

    // 规范化模型别名
    const normalizedModel: string = VALID_MODELS.includes(model) ? model : 'haiku';

    // 使用 resolveModel 获取实际模型ID
    const resolvedModel = ApiKeyService.resolveModel(normalizedModel);

    // Inject model identity
    const modelLabel: string = normalizedModel === 'opus' ? 'Opus（最强推理模型）'
    ...
    const effectiveSystemPrompt: string = finalSystemPrompt + identityPrompt;
    ...
    try {
      ...
      const queryOptions: any = {
        ...
        // 只启用只读工具：WebSearch、WebFetch、Read、Glob、Grep
        tools: [
          { type: 'web-search' },
          { type: 'web-fetch' },
          { type: 'file-read' },
   ...
   * 检查标题是否为默认值
   */
  static _isDefaultTitle(title: string): boolean {
   ...
   * 自动生成会话标题
   */
  static async _generateTitle(sessionId: string, messages: ChatMessage[]): Promise<void> {
    const conversationSummary: string = messages
      .filter(m => m.role !== 'system')
      .map(m => `${m.role === 'user' ? '用户' : 'AI'}: ${m.content.substring(0, 100)}`)
      .join('\n');

    const prompt: string = `请根据以下对话内容，生成一个简短的标题（不超过20个字）。只输出标题本身，不要添加任何其他内容：\n\n${conversationSummary}`;

    try {
      const title: string = await this._callClaude(sessionId + '-title', prompt, 'haiku',
        '你是一个标题生成器。根据对话内容生成简洁的中文标题，不超过20个字。只输出标题。');

      const cleanTitle: string = title.replace(/["'""「」【】]/g, '').trim().substring(0, 30);
      if (cleanTitle) {
