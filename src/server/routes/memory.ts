...
router.get('/shared/pool', (req: any, res: any, next: any) => {
  try {
    res.json({ success: true, data: MemoryService.getSharedPool() });
  } catch (err) { next(err); }
});

router.put('/shared/pool', (req: any, res: any, next: any) => {
  try {
    const data = req.body;
...
router.get('/:workflowId/runs', async (req: any, res: any) => {
  try {
    const memory = MemoryService.getMemory(req.params.workflowId);
    if (!memory || !memory.trim()) {
      return res.json({ success: true, data: [] });
    }
    const sections = memory.split(/(?=## Session )/).filter((s: string) => s.trim());
    const entries = sections.map((s: string) => {
...
router.get('/:workflowId', (req: any, res: any, next: any) => {
  try {
...
router.put('/:workflowId', (req: any, res: any, next: any) => {
  try {
...
router.delete('/:workflowId', (req: any, res: any, next: any) => {
  try {
