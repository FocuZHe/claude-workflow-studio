...
router.post('/', (req: any, res: any, next: any) => {
  try {
    if (!req.body.title) throw new AppError('VALIDATION_ERROR', 'title is required', 400);
...
router.put('/:id', (req: any, res: any, next: any) => {
  try {
    ...
    if (!result) throw new AppError('NOT_FOUND', 'Not found', 404);
...
router.delete('/:id', (req: any, res: any, next: any) => {
  try {
    if (!KnowledgeService.delete(req.params.id)) throw new AppError('NOT_FOUND', 'Not found', 404);
...
router.post('/tags', (req: any, res: any, next: any) => {
  try {
    if (!req.body.name) throw new AppError('VALIDATION_ERROR', 'name is required', 400);
    const tag = TagService.create(req.body.name, req.body.color);
    if (!tag) throw new AppError('CONFLICT', '标签已存在', 409);
    res.status(201).json({ success: true, data: tag });
  } catch (err) { next(err); }
});

router.delete('/tags/:id', (req: any, res: any, next: any) => {
  try {
    if (!TagService.delete(req.params.id)) throw new AppError('NOT_FOUND', 'Not found', 404);
    res.json({ success: true });
  } catch (err) { next(err); }
});
...
router.get('/export', (req: any, res: any, next: any) => {
  try {
    ...
    if (format === 'csv') {
      ...
      const md = entries.map((e: any) => {
        let text = `# ${e.title}\n\n`;
        if (e.category) text += `分类: ${e.category}\n`;
        if (e.tags?.length) text += `标签: ${e.tags.join(', ')}\n`;
        text += `\n${e.content}\n\n---\n`;
        return text;
      }).join('\n');
  ...
  try {
    ...
    for (const e of entries.slice(0, 500)) { // max 500
      ...
      const entry = KnowledgeService.add({
        ...
        tags: Array.isArray(e.tags) ? e.tags : (typeof e.tags === 'string' ? e.tags.split(/[;,]/).map((t: string) => t.trim()).filter(Boolean) : []),
        source: 'import'
      });
      imported.push(entry);
    }
