...
router.post('/',
  validate(
    ...
    validateEnum('priority', ['low', 'medium', 'high', 'urgent'])
  ),
...
router.put('/:id',
  validate(
    ...
    validateString('description', 0, 2000),
    validateEnum('priority', ['low', 'medium', 'high', 'urgent']),
    validateEnum('status', ['pending', 'running', 'completed', 'failed', 'cancelled'])
  ),
  (req: any, res: any, next: any) => {
    try {
 ...
 */
router.post('/:id/execute', async (req: any, res: any, next: any) => {
  try {
 ...
 */
router.post('/:id/cancel', (req: any, res: any, next: any) => {
  try {
