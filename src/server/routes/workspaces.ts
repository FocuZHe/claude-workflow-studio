...
router.get('/', (req: any, res: any) => {
  ...
  const enriched = workspaces.map((ws: any) => {
    ...
    try {
      ...
      count = workflowList.filter((wf: any) => wf.workspaceId === ws.id).length;
    }
    return {
      ...ws,
      workflowCount: count
 ...
 * 获取指定工作区的状态信息
 */
router.get('/:id/state', (req: any, res: any, next: any) => {
  try {
 ...
 */
router.get('/:id/workflows', (req: any, res: any, next: any) => {
  try {
