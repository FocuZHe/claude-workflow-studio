...
router.post('/reindex', (req: any, res: any, next: any) => {
  try {
...
// 获取文件内容
router.get('/:id/content', (req: any, res: any, next: any) => {
  try {
    const { id } = req.params;
    const FileService = require('../services/FileService');
    const workspaceRoot = FileService.getWorkspaceRoot();
    ...
    // 在索引中查找文件
    const artifact = ArtifactIndexService._index.find((a: any) => a.id === id);
    if (!artifact) {
