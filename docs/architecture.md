...
| **工作流** | 可视化拖拽编排，支持 6 种节点类型（含条件分支）、AI 创建、批量克隆、流程图导入/导出、记忆传递、知识注入 |
...
| 特性 | 说明 |
|------|------|
| 状态机 | pending → running → completed/failed/cancelled/paused |
| 优先级 | low / medium / high / critical |
| 执行模式 | 单任务执行 或 关联工作流执行 |
| 自动修复 | Token/频率错误自动切换备用模型重试 |
| 并发控制 | 防止同一任务重复执行 |
| 超时机制 | 默认 30 分钟超时，可自定义 |
| 实时广播 | 状态变更通过 WebSocket 实时推送 |

#### TaskQueueService — 任务队列管理

| 特性 | 说明 |
|------|------|
| 队列状态 | idle / running / paused / completed / cancelled |
| 批量执行 | 按优先级+时间排序依次执行 |
| 暂停/恢复 | 支持暂停队列和恢复执行 |
| 状态广播 | 队列事件通过 `queue.{event}` 推送 |
| 事件类型 | started / paused / resumed / completed / failed / cancelled |

#### ChatService — AI 对话服务

| 特性 | 说明 |
|------|------|
| 执行引擎 | Claude Agent SDK（query() 调用） |
| 会话管理 | 创建/归档/搜索会话 |
| 上下文控制 | maxMessages / maxTokens / summarizeOld |
| 流式输出 | 实时推送每个 token |
| 会话复用 | 支持 resume 复用已有会话 |
| 会话持久化 | SDK 会话 ID 持久化到 ChatSession，服务器重启后可恢复 |
| 历史恢复 | 当 SDK 会话不可用时，自动将历史消息包含在 prompt 中 |
| 工作区隔离 | 每个对话在独立工作区执行 |

#### TerminalService — PTY 终端服务

| 特性 | 说明 |
|------|------|
| 实现方式 | node-pty 真实 PTY 进程 |
| 多会话 | 支持最多 10 个终端会话 |
| 实时输出 | PTY 输出通过 WebSocket 实时推送 |
| 自动路径 | 在当前工作区路径自动打开 |
| 跨平台 | Windows (cmd.exe) / Linux/macOS (bash) |
...
|------|------|
| 工作区沙箱 | 所有操作限制在工作区内 |
| 路径校验 | resolvePath 防止路径穿越 |
| 原子写入 | atomicWriteSync 防止数据损坏 |
| 执行追踪 | 按 runId 追踪活跃执行 |
| 文件监听 | 支持文件变化监听 |

#### SafetyService — 安全规则管理

| 特性 | 说明 |
|------|------|
| 规则管理 | 创建/更新/删除安全规则 |
| 安全评分 | 基于启用规则数量计算（0-100 分） |
| 默认规则 | 内置速率限制规则 |
