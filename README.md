...
一个基于 Web 的可视化管理平台，用于创建、编排和控制多个 Claude Code Agent，通过工作流将它们串联起来完成复杂任务。

简单说：一个**网页版 AI 工厂调度中心**。Agent 是工人，工作流是流水线，你就是厂长。你可以创建多个 AI 助手，每个配置不同的模型和能力，把它们连成流水线，第一个干完自动交给下一个，实时看到每个 AI 在干什么、输出了什么。
...
┌──────────────────────────────────────────────────────────┐
│                    浏览器 (SPA)                           │
│  HTML + CSS + Vanilla JS + xterm.js + SVG 图标            │
└────────────────────┬─────────────────────────────────────┘
                     │  HTTP REST + WebSocket
┌────────────────────┴─────────────────────────────────────┐
...
│  └───────────────────┘    └──────────────────────────┘   │
│                                                            │
│  回退：CLI 不可用时，子 Agent 自动回退到 SDK 模式          │
└────────────────────┬─────────────────────────────────────┘
                     │
┌────────────────────┴─────────────────────────────────────┐
│                    数据层                                  │
│  sql.js (WASM SQLite) │ JSON 文件 │ 工作区文件            │
└──────────────────────────────────────────────────────────┘
```
...
| 节点 | 说明 |
|------|------|
| **开始 (start)** | 工作流入口，透传用户输入 |
| **Agent** | spawn `claude --print` 子进程，注入节点专属的模型/技能/提示词 |
| **条件 (condition)** | 执行表达式判断，分支路由，级联跳过传播 |
| **并行 (parallel)** | 单次消息原子派发多个 Agent |
| **审批 (approval)** | WebSocket 往返 — 暂停等待用户审批通过/拒绝 |
| **合并 (merge)** | 收集直接上游输出，用 `---` 分隔合并 |
| **子工作流 (subworkflow)** | 递归调用其他工作流，记忆回传父工作流 |
| **代码 (code)** | VM 沙箱执行 JS 代码（10 秒超时，无 fs/网络） |
| **结束 (end)** | 汇总上游输出，生成最终结果 |
...
| **工作流** | 可视化拖拽编排，支持 10 种节点类型、AI 创建、批量克隆、流程图导入/导出、记忆传递、知识注入 |
