...
- **记忆系统**：任务标签记忆、关键词过滤注入、`[记忆: xxx]` 主动提取、跨工作流传递、共享数据池、自动压缩
- **Claude Code Workflows 互操作**：导入 `.md` 工作流到可视化编辑器，导出可视化工作流为 `.md` 格式
...
| **记忆** | 任务标签记忆、关键词过滤注入、`[记忆: xxx]` 主动提取、跨工作流传递、共享数据池、自动压缩 |
...
| 模块 | 说明 |
|------|------|
| **市场** | 技能市场（249 个技能，19 个分类）+ 工作流模板（19 个内置），支持搜索、分类筛选、多选安装 |
| **广播** | 事件广播和通知管理 |
